...
export const REGISTER = [
  ...
  {
    id: "ucp.finality.no-storewide-count",
    file: "src/lib/ucp/payments/usdc-x402.ts",
    match: "There is no storewide confirmation count",
    resolution: "derived",
    from: "RAIL_FINALITY in the same file is keyed per rail and holds no count; usdcPaymentHandlers copies each rail's own entry into its instance",
    why: "A claim that a number does not exist, made in the file that would hold it. There is nowhere else for a storewide count to live: the handler config is built only from RAIL_FINALITY, so the day somebody adds a count the sentence becomes false in the same file that made it. test/ucp/profile.spec.ts asserts no confirmations figure appears in any emitted handler, so the build fails rather than the sentence going quietly wrong. Not a dating case — it is structural, not an observation that expires.",
  },
  {
    id: "ucp.inputs.no-second-copy",
    file: "src/routes/ucp.ts",
    match: "There is no second copy of the validation rules",
    resolution: "derived",
    from: "the same route calls buyInputSchema(item) to build every per-item schema it serves, and src/lib/ucp/catalog.ts calls it for the variant metadata",
    why: "The sentence is a claim about this store's own architecture, and the code beside it is the proof: /ucp/schemas/items/{id}.input.json is generated by calling buyInputSchema, the identical function the 402 body, the MCP tool schema, the Bazaar entry and the OpenAPI spec are generated from. test/ucp/routes.spec.ts compares the served schema against buyInputSchema directly, so a hand-written copy appearing here fails the build. The defect this guards against already happened once, to graffiti's tag on the MCP shelf.",
  },
  {
    id: "bounties.plan-run.nothing-to-press",
    file: "src/routes/admin.ts",
    match: "There is no standing order to press",
    resolution: "derived",
    from: "the handler reads readBountyPlan(c.env) on the line above and answers this only when the plan is absent or its runs_remaining has reached 0",
    why: "A claim that a thing does not exist, made by the door that just looked for it. It is not an observation that expires: the sentence is the plan read, rendered — the same read the pass itself makes, so a plan that exists can never produce it. test/bounty-plan.spec.ts presses this door with no plan written and requires the 409, and presses it with a plan and requires the pass instead, so the sentence fails the build the day it stops following the read.",
  },
];
