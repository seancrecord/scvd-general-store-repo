import assert from "node:assert/strict";
import { readFileSync } from "node:fs";
import test from "node:test";
import { SELF_PUBLISHED_IDS, citationsOn, exitCodeFor, judge } from "./lib/citations.mjs";

const BASE = "https://scvd.store";

test("a verify URL or a corpus ROW on the page is a citation; the store's other pages are not", () => {
  const page = `<p>Row: <a href="${BASE}/api/verify/cert_theirsnotours">verify</a> and ${BASE}/corpus/host/door.example.json and ${BASE}/corpus/3.json and ${BASE}/corpus/round/2026-W36</p><a href="${BASE}/menu/hello">shop</a>`;
  assert.deepEqual(citationsOn(page, BASE), [
    `${BASE}/api/verify/cert_theirsnotours`,
    `${BASE}/corpus/3.json`,
    `${BASE}/corpus/host/door.example.json`,
    `${BASE}/corpus/round/2026-W36`,
  ]);
  assert.deepEqual(citationsOn(`<a href="${BASE}/menu/hello">shop</a>`, BASE), []);
});

test("our own words on their page are not a citation: the bare corpus link from the README, the sample certificate from every listing", () => {
  // Verbatim from README.md, as Glama mirrored it on 2026-09-04.
  const mirrored = `read the dated, Bitcoin-anchored corpus, free, at <a href="${BASE}/corpus">scvd.store/corpus</a>, cite it by DOI`;
  assert.deepEqual(citationsOn(mirrored, BASE), []);
  assert.deepEqual(citationsOn(`${BASE}/corpus.json ${BASE}/corpus/latest.json ${BASE}/corpus/diff.json?since=2026-W30`, BASE), []);
  for (const id of SELF_PUBLISHED_IDS) {
    assert.deepEqual(citationsOn(`sample_verify_url: ${BASE}/api/verify/${id}`, BASE), []);
  }
});

test("a page that still cites is cited; one that stopped is gone; one we could not read is unreadable and not a failure", () => {
  const system = { name: "Example Scores", cites_at: "https://scores.example/method", since: "2026-09-03" };
  assert.equal(judge(system, { status: 200, text: `see ${BASE}/api/verify/cert_9pq2rstuvw` }).verdict, "cited");
  assert.equal(judge(system, { status: 200, text: "we score things" }).verdict, "gone");
  assert.equal(judge(system, { status: 503, text: "" }).verdict, "unreadable");
  assert.equal(judge(system, { error: "ECONNRESET" }).verdict, "unreadable");
  assert.equal(exitCodeFor([judge(system, { status: 200, text: "nothing" })]), 1);
  assert.equal(exitCodeFor([judge(system, { error: "down" })]), 0);
  assert.equal(exitCodeFor([]), 0);
});

test("the register the page renders is the register the check reads, and every entry is complete", () => {
  const register = JSON.parse(readFileSync(new URL("../src/store/citing-systems.json", import.meta.url), "utf8"));
  assert.ok(Array.isArray(register.systems));
  for (const entry of register.systems) {
    assert.equal(typeof entry.name, "string");
    assert.match(entry.cites_at, /^https:\/\//);
    assert.match(entry.since, /^\d{4}-\d{2}-\d{2}$/);
  }
});

test("the first sweep's three findings were all our own words or a clipped URL", () => {
  for (const id of SELF_PUBLISHED_IDS) {
    assert.deepEqual(citationsOn(`"verify_url":"${BASE}/api/verify/${id}"`, BASE), []);
  }
  for (const clipped of [`${BASE}/api/verify/ce`, `${BASE}/api/verify/cert_`]) {
    assert.deepEqual(citationsOn(clipped, BASE), []);
  }
  assert.deepEqual(citationsOn(`${BASE}/api/verify/cert_9pq2rstuvw`, BASE), [`${BASE}/api/verify/cert_9pq2rstuvw`]);
});
