...
## 2026-09-11 — The probe rule restored; input validation kept for composed requests

Re-read the [x402 v2 core payment flow](https://github.com/coinbase/x402/blob/main/specs/x402-specification-v2.md) and the [Bazaar discovery extension](https://github.com/coinbase/x402/tree/main/typescript/packages/extensions): the flow a client implements is request, 402, sign, retry the same URL. Nothing in it reads a catalog first, and the discovery extension exists precisely so a 402 can declare the input a resource needs. The store's own free preflight (`src/services/preflight.ts`, the `status-402` check) and its defect vocabulary (`inputs-undeclared`: "declare required parameters in the challenge itself, before payment") say the same. Seven hours of the BUY-002 policy from the previous entry answered 25 of 32 bare probes with 400, and x402-list's checker read that as 25 missing endpoints. A bare unsigned request missing a required input is a probe again and answers 402 with `required_params`, `required_params_note` and `input_contract_url` in the body. What BUY-002 rightly wanted is kept for every other request: signed, or unsigned with every required input supplied, inputs are validated in full before terms. Regression coverage: every menu item, both Workers, bare probe answers 402; every item with required inputs, supplied-but-invalid answers 400 without terms; signed missing inputs still fail before verification.
...
## 2026-09-13 — Launch Check exact authorization and replay evidence

Read [ERC-3009](https://eips.ethereum.org/EIPS/eip-3009): the authorization identifies its payer, recipient, value and nonce within the token and chain domain. Nonces are scoped to the authorizer; finding a nonce somewhere in a multi-payment receipt is insufficient. Circle's [EIP3009 implementation](https://github.com/circlefin/stablecoin-evm/blob/fc85788bc7c23cefe3df1a757133048bfddadeaa/contracts/v2/EIP3009.sol) marks the authorization used immediately before transferring. Its [FiatTokenV1 transfer implementation](https://github.com/circlefin/stablecoin-evm/blob/fc85788bc7c23cefe3df1a757133048bfddadeaa/contracts/v1/FiatTokenV1.sol) emits the Transfer after updating balances. The repository head was resolved through GitHub's public API to the revision linked here. This supports pairing adjacent canonical-token receipt events for this instrument; it is not a generic guarantee about every ERC-3009 implementation. Unrecognized ordering remains unconfirmed. RPC identity, chain, receipt success, block/head shape and confirmation depth are separate checks. These are trusted-RPC observations, not independent consensus proofs or verification of the deployed proxy's implementation at every historical block.

The same single-use authorization may settle on either presentation; a 2xx alone cannot tell which did. Comparing complete raw response bytes establishes byte recovery only. Different wrapper metadata can surround the same artifact, and repeated transaction references can accompany different artifacts. UTF-8 decoding can map different invalid byte strings to the same text, so the digest precedes decoding. Truncated digests describe prefixes. These are measurement limits, not new protocol requirements. Re-read [Workers best practices](https://developers.cloudflare.com/workers/best-practices/workers-best-practices/) for bounded response reads and awaited work. No dependency, binding, secret, wallet funding or deployment was added. The separate general Settlement Attestation nonce classifier is unchanged and tracked for follow-up.

The saved September 11–12 deployment benchmark supplies an additional implementation check: all 18 receipts in `research/deployment-boundary-2026-09-11/proofs.json` contain the adjacent canonical-USDC authorization/transfer pair with the recorded payer and amount. The token and event constants were derived from `src/lib/base-rpc.ts`. This is a reread of existing evidence, not a fresh RPC observation or new purchase; it does not qualify other implementations or future upgrades.


## 2026-09-13 — general attestation correspondence

Re-read [ERC-3009](https://eips.ethereum.org/EIPS/eip-3009) and the pinned [Circle EIP3009 implementation](https://github.com/circlefin/stablecoin-evm/blob/fc85788bc7c23cefe3df1a757133048bfddadeaa/contracts/v2/EIP3009.sol) cited above. Nonce uniqueness is per authorizer; optional payer/recipient/amount query fields cannot be satisfied by independent legs of a batch. The general Settlement Attestation now shares Launch Check's canonical-USDC event-pair interpretation, with optional fields still optional. Multiple candidate authorizers without a payer are ambiguous; unknown event order remains unestablished. This does not transplant Launch Check's separate RPC chain/identity/head checks into the general reader. Re-read Workers best practices and inspected installed Worker types before implementation; no binding, dependency or config changes.

## 2026-09-14 — reconciliation pairing and general receipt context
