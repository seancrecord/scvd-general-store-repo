...
The release-status recheck on September 15 used main at
`b62b30b2d2c76e2386d70016ef8519c098607b14`, PR base branches and Git ancestry.
Census/passport [PR #691](https://github.com/seancrecord/scvd-general-store-repo/pull/691)
and discovery comparison [PR #712](https://github.com/seancrecord/scvd-general-store-repo/pull/712)
are integrated into main. The core draft-01 reader
[PR #715](https://github.com/seancrecord/scvd-general-store-repo/pull/715)
was merged into `codex/mpp-discovery-comparison-pr` after #712 merged to main;
its merge commit `da9b777ee286160c95731bf7544ad7f779fbe21b` is not an ancestor
of this main revision, and the core reader is absent there. A merged PR
status alone did not establish release. These readers observe other services'
MPP challenges; none makes the store accept MPP payments.

The separate **Audit cold-entry buyer journeys** task owns buyer-path work.
[PR #714](https://github.com/seancrecord/scvd-general-store-repo/pull/714)
was likewise merged into `codex/agent-catalog-readability`, not main. Its
merge commit `ed5f9433480a263bde091d9390148f5c25a1ccfa` is not an ancestor of
the checked main revision. The change preserving `Purchase-Recovery` as
`purchase_recovery` in WebMCP publication results has not reached that tree.
Its reported live quote and mocked stock-client checks do not establish a
paid WebMCP or CDP-managed wallet purchase. Reconcile and integrate the
existing changes before claiming their release; do not rebuild them.
...
- **Defect vocabulary (`defects/`):** the MPP branches include generated
