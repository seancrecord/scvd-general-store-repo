...
| 7. Framework examples | Nothing. | One `examples/` directory: OpenAI Agents (tool schema + remote MCP), Vercel AI SDK, LangChain/LangGraph, CrewAI, PydanticAI, AutoGen, a Claude Code / Cursor skill file, a Copilot-friendly README; each operational ("an agent is about to pay; it calls preflight; it reads network, asset, recipient and the defect state; it decides"), each tested in CI against fixtures. | C2 |
