...
| 1.2 [~]                    | B9, G1, I5     | **PARTIAL 2026-08-25 (`b7d9354` #246): standing-watch producer landed.** Ward rounds and launch checks remain open.                                                                                | Red test: current main discards evidence; new artifacts carry and store it                                                             |
| 1.3 [~]                    | D6             | Methodology/battery version + schema id INSIDE signed bytes on all observation classes                                                                                                            | Envelope slice: validator refuses a missing battery_version; join wraps derive it. Attestations / launch-check / watch rows still open |
...
| 1.j8 | `freshness_coherence` catalog-only: as_of + valid_until                                       | ✅ 2026-08-25 `e9db33b` #249                                  |
| 1.j9 | Next batteries: registry. price_settlement is 0.17's path. identity_binding ⚑ G2              | Open                                                         |
...
| #   | Ledger       | What                                                                                                                                                   | Acceptance                                                                                     |
| --- | ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- |
| 2.1 ✅ 2026-08-26 `e83d676` `1d00a23` #264 #266 | B1/B2/B3, A3 | Probe-layer honesty: reached_level, tri-state checks vector, split `unreachable`, conditions beside every verdict                                      | Red tests from B-adversarial list                                                              |
| 2.2 ✅ 2026-08-26 `dada49e` #267 | B13, F1, I1  | ONE shared value-checks module (canonical USDC asset, CAIP-2, atomic amounts, payTo, testnet) consumed by battery + desk + verdict fold + launch check | Red test: hostile 402 naming arbitrary ERC-20 currently signed as "USDC" — refused after       |
| 2.3 ✅ 2026-08-26 `825cd0c` #268 | B16, E2, L4  | Battery release discipline: stable check IDs, `checks.json` derived from code, endpoint fixtures (recorded bytes, offline replay), changelog           | Manifest generator refuses hand-typed drift; every known-bad fixture red for exactly one check |
| 2.4 ✅ 2026-08-26 `e4f0545` #269 | F2, I6       | Offer/signer validation at the desk; launch check consumes it (absence recorded either way)                                                            | Red tests both consumers                                                                       |
| 2.5 ✅ 2026-08-27 `4778e22` #271 | B12, H4      | Absence-stated law at remaining surfaces (fresh-set conditions)                                                                                        | Copy/shape tests                                                                               |
...
| 3.2 ✅ 2026-08-27 `70e21bf` #280 | C2, I4       | Money-path symmetry: verify seller-claimed tx on chain via the existing attestation reader; until read, label `tx_hash_claimed` | Red test: fabricated PAYMENT-RESPONSE hash currently signed as fact |
