...
- [ ] **BUY-017 — SEV-1 fault case: lost settlement acknowledgement can leave no artifact and report “No charge”** — partial: verified purchase intents, truthful payment status and original-evidence recovery are implemented for the checked product families below. All current catalogue products and newly recorded commission/publication purchases are now admitted; historical obligations now have evidence-backed refund resolution; remaining legacy retry paths still need proof that missing original goods are never replaced.
...
- [ ] **BUY-037 — SEV-1: MCP cannot reconstruct some settled purchases even with the original key** — partial: HTTP and both MCP profiles recover the checked product families from authenticated purchase records. All current catalogue products and newly recorded commission/publication purchases are now admitted; older purchases missing original input/evidence bindings now have evidence-backed refund resolution; safe legacy retry handling remains open.
...
The two remaining recovery parents need a product-by-product finish line. Existing complete coverage is named by `supportsArtifactRecovery()` in `src/lib/artifact-checkpoint.ts`. Partial progress and CI repairs must stay visible without counting either as a fully closed buyer finding. A 2026-09-09 snapshot derived from `MENU_ITEMS.filter(supportsArtifactRecovery)` admits all 33 current catalogue products to artifact recovery. That catalogue count does not include commission/publication doors; their newly recorded purchase recovery is now complete and documented below. The open parents retain historical obligations without original goods or inputs.
...
- **BUY-017:** current catalogue coverage and newly recorded commission/publication recovery are complete. Finish historical obligations, including older commissions and publications, after an ambiguous settlement answer. A truthful unknown/settled status is necessary but does not itself deliver the good.
...
- **BUY-037:** current catalogue coverage and newly recorded commission/publication recovery are complete. Finish equivalent authenticated recovery or explicit resolution for older purchases whose original input/evidence was never retained. The original payment must not buy replacement evidence or trigger a second charge.

Commission delivery now has its own original-brief and order-checkpoint proof below. Concurrent fresh payments racing one quote and human capacity reservations remain separate admission work; this recovery step does not close them. Missing private recovery handles and expired/rejected verification also remain tracked under BUY-014/015; they must not disappear merely because a product's reconstruction substep passes. Parent closure needs explicit evidence for the remaining obligation or a documented resolution, not an unchecked promise to reconstruct data the store never retained.
