...
- [x] Expired empty-watch history wording — **repaired September 12 on `codex/buyer-watch-expiry`.** The empty history now says no observations were recorded and the term has ended. Active watches retain their upcoming-probe explanation; the original dates and missed-hour count are unchanged.

September 12 follow-up evidence: `test/watch-empty-history.spec.ts` checks the actual HTML and JSON responses for active, ended and recovered-after-expiry histories. The recovery case deletes the public record, observes its 404, and restores it through the existing durable publication path before reading again. Both ended-history regressions fail on the original source; the active control passes. With the repair, all 27 related tests across three files pass, along with typecheck and both Worker dry-run bundles. The completed full run covered 686 files: 13,443 tests passed, 11 failed and one existing test was skipped. An eight-file rerun passed 1,227 tests but encountered seven different failures; all 18 cases that failed in either run then passed serially in 25.46 seconds, without changing assertions or timeouts. This is a completed full run plus successful targeted reruns, not an uninterrupted green full run. The separate Aura/method gate passed 21 tests; docs checks passed. These are local presentation/recovery checks, not a new live purchase or deployment. The original 39-finding completion count is unchanged; this was an explicitly retained follow-up.
...
- [x] Public report response contracts — **repaired September 12 on `codex/buyer-report-contract`.** Launch Check now describes its served `certificate` URL. Launch Check, Payment Dry Run and Settlement Reconciliation each advertise their own verdict vocabulary rather than the endpoint-audit enum. The existing service-audit and on-page-audit vocabularies remain constrained; case files no longer advertise an audit verdict they do not produce.

September 12 follow-up evidence: three actual HTTP report responses fail validation against the original served OpenAPI document: Launch Check (`no_payment_gate`, plus the absent required `cert_id`), Payment Dry Run (`cannot_simulate`), and Settlement Reconciliation (`no_settlement`). `test/report-response-contract.spec.ts` constructs these signed observations through the real services using local target/RPC fixtures, stores them, and validates what the public routes serve. Two audit controls pass on both versions. A later focused regression also rejects the Payment Dry Run description of a wallet's “settled purchases”; its contract now describes observed terms and declared-client simulation without payment. With the correction, all five report cases pass and still reject invented verdicts, non-string signatures and missing certificate references. The initial four-file contract gate passes 95 tests. Full-suite review caught the added enum explanations exceeding the existing OpenAPI reader budget; the repair keeps the checked enum keys without that extra prose, and the budget is unchanged. The final five-file gate passes all 99 tests, including the later description guard and byte-budget check. Typecheck checks each advertised outcome map exhaustively against its instrument's type. The completed full attempt recorded 684 files: 13,423 tests passed, 27 failed and one existing test was skipped, with two additional worker-start failures. The description and size guards were corrected during validation. All 27 failed cases then passed serially in 51.81 seconds, and both previously unstarted suites passed all seven tests. Final typecheck, both Worker dry-run bundles and docs checks pass. This is a completed full attempt plus successful reruns of every failed case and unstarted suite, not an uninterrupted green full run. These checks do not purchase anything or establish live settlement/certificate retrieval. The original 39-finding count is unchanged; this closes a retained follow-up, with two additional affected report types reproduced during repair.
- [x] **Closed by battery v4; box ticked 2026-09-19.** `readReplay` in `src/services/launch-check.ts` no longer produces `served_again` at all (the outcome is retained only to read historical v3 records): identical complete bytes read `redelivered`, changed bytes read `changed_response` with "fresh fulfillment is not established" in the stage line, and a transaction reference alone establishes nothing (`launch-check-terms.ts`). The original text follows. Distinguish idempotent retrieval from repeated fulfillment in the launch replay verdict. Main now carries battery v3: a 2xx naming the original settlement is classified as redelivery. The remaining gap is a 2xx without that transaction reference: it still becomes `served_again` without establishing fresh work, including a possible original-artifact retrieval. Merely repeating the original transaction reference also does not prove the delivered artifact stayed the same. Preserve this follow-up until both boundaries have evidence.
...
- [x] Launch Check recipient guidance now names the actual `tx_hash_status` values and explains their limits. The status map is checked exhaustively against the service type; served reports cover settled, pending-finality, not-found, reverted, insufficient-match, malformed-identifier, missing-receipt and unpaid cases. The guide separates a transfer read from exact authorization settlement, gives the signature-byte construction and current/retired key lookup, and cautions against signing a fresh payment after uncertainty. This closes the guidance follow-up only; exact settlement correlation and replay interpretation above remain open. See the September 12 validation entry below.
...
## Launch Check recipient guidance — 2026-09-12

New work in this repair: corrected the nonexistent `confirmed` status in both the public verification guide and the delivery note, added the omitted contradictory/unreadable receipt outcomes, preserved the distinction between a transfer and settlement of this exact purchase, and made signature verification and historical-key attribution explicit. A dated entry at `/corrections` withdraws the overstated guide; saved signed artifacts are unchanged. These instructions explain the existing observations rather than manufacturing missing evidence.

Evidence: all eight new served-report regression cases failed against unchanged production code. The initial four-file focused gate passed 54 tests. The subsequent surface sweep reproduced the same status typo in the delivery note: two additional tests fed actual settled/pending-finality reports into the note and failed against the old branch. The note now uses the instrument's status type and describes the observed transfer without claiming exact purchase settlement. The older note fixtures that supplied the nonexistent status were corrected to the actual emitted value. One fixture deliberately returns a transfer for a different amount, proving the guidance must keep `payment_attempt.settlement: unknown` visible even beside `confirmed_on_chain`. The source read for the authorization boundary is logged in `docs/SPEC_READS.md` under September 12. The recipient test resolves the public key registry, verifies the actual served bytes with the raw ed25519 library, and rejects a changed subject. It uses a local signer, local seller and injected receipt reads; it neither spends live funds nor retrieves a real purchase certificate. This is regression evidence, not another cold-agent Aura Walk.

Validation complete: the final full suite passed all 694 files, with 13,538 tests passed and one existing key-continuity skip (2,886.09 seconds; four workers). No failed-case reruns were needed. Typecheck, both Worker dry-run bundles, documentation checks and both correction-index tests pass. The final five-file focused gate passes all 60 tests, including ten new recipient/note cases and the collector dependency boundary. The first full run was deliberately interrupted when the delivery-note counterpart was found; it is not counted as a completed run. The original 39-finding count is unchanged; this closes one retained follow-up.

Live follow-through: the September 13 UTC eight-request recheck observed the
prior BUY-042/044 discovery repairs on production: both literal examples quote
