...
**22/39 original findings are complete, including all 6/6 SEV-1s.** Existing catalogue recovery coverage is derived from `supportsArtifactRecovery()` and `MENU_ITEMS`; all 33 current products are admitted. Newly recorded commission/publication recovery, historical original-order retrieval, evidence-backed resolution and the final legacy retry guard complete the recovery parents. Atomic admission also prevents concurrent fresh authorizations from charging the same new keyed purchase twice. Remaining P1/P2 findings below retain their own scope, including missing recovery handles and expired verification.
...
These are tested repair mechanisms, not production-customer inventory or live-refund claims. Missing private handles and expired/rejected verification remain BUY-014/015. Capacity reservations and ordinary signatures binding human work to its brief remain separate findings.
