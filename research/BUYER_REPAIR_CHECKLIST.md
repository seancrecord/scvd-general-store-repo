...
- [ ] **BUY-017 — SEV-1 fault case: lost settlement acknowledgement can leave no artifact and report “No charge”** — partial: verified purchase intents, truthful payment status and original-evidence recovery are implemented for the checked product families below. All current catalogue products and newly recorded commission/publication purchases are now admitted; historical obligations still need recovery proof or explicit resolution.
...
- [ ] **BUY-037 — SEV-1: MCP cannot reconstruct some settled purchases even with the original key** — partial: HTTP and both MCP profiles recover the checked product families from authenticated purchase records. All current catalogue products and newly recorded commission/publication purchases are now admitted; older purchases missing original input/evidence bindings remain open.
