...
**21/39 original findings are complete, including all 6/6 SEV-1s.** Existing catalogue recovery coverage is derived from `supportsArtifactRecovery()` and `MENU_ITEMS`; all 33 current products are admitted. Newly recorded commission/publication recovery, historical original-order retrieval, evidence-backed resolution and the final legacy retry guard complete the recovery parents. Remaining P1/P2 findings below retain their own scope, including missing recovery handles, expired verification and concurrent fresh authorizations.
...
These are tested repair mechanisms, not production-customer inventory or live-refund claims. Missing private handles, expired/rejected verification and concurrent fresh payments remain BUY-014/015/016. Capacity reservations and ordinary signatures binding human work to its brief remain separate findings.
