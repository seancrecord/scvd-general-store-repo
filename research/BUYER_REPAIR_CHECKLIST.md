...
- [ ] Launch Check verification instructions name `tx_hash_status: "confirmed"`, while the service emits `confirmed_on_chain`. Found during the September 12 public-contract repair in `src/routes/launch-check.ts` versus `TxHashStatus` in `src/services/launch-check.ts`. Align the recipient-facing vocabulary and test the actual served instructions alongside a report with chain evidence. This is a newly recorded follow-up; the schema repair does not close it.
