...
fs.writeFileSync(path.join(out,'protocol.json'),JSON.stringify({date:new Date().toISOString(),modelRequested:model,variants,modes:['tasks'],cases:cases.map(c=>c.id),triggers,attemptsPerCell:1,timeoutMs:240000,resume:false,rescue:false,scope:'One separately recorded corrective task session on the integrated skill; two retained fixture-backed cases: timestamp reporting and MPP interpretation. No metadata recheck or replacement cohort. Cases within a task session share context; not native host auto-triggering, real endpoint execution, or statistical reliability.',sourceHashes:Object.fromEntries(['acceptance.json','cases.json','eval-tool.mjs','run-readers.mjs','score-readers.mjs'].map(n=>[n,hash(fs.readFileSync(path.join(root,n)))]))},null,2)+'\n');
for(const mode of ['tasks']) await Promise.all(variants.map(v=>run(v,mode)));
