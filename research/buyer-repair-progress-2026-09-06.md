...
## 2026-09-07 — BUY-017: catalogue intent capture and private status

Built independently on `codex/buyer-durable-intent`, based on #541 at `9b976ab3`. Before a catalogue settlement attempt, both HTTP and MCP durably retain the full request (not the 600-character desk preview), selected payment requirements, item snapshot, verified payer, acceptance time and EVM authorization window/nonce. Signed payment payloads are not retained. One atomic record per verified payment identity admits one attempt; an unresolved retry cannot resubmit payment or replace the original inputs. Capture/read failure submits no payment on that request and conservatively leaves the charge state unknown because an earlier request may exist.

Unknown responses carry a stable purchase ID, private status token, HTTP status URL and `check_purchase` MCP tool name. Both status doors share the same reader, never submit payment, and remain available after authorization expiry. Invalid tokens reveal neither record existence nor its inputs. Existing paid cache/artifact retrieval remains ahead of the unknown-state guard; an unknown human purchase is answered before closed-shelf admission. A failed status update after confirmed settlement does not interrupt delivery. The derived browser handler and curl example put the capability in Authorization, not the query string; OpenAPI declares bearer authentication separately from price. The privacy page names retention without automatic expiry.

Validation: all 32 final new cases failed on prior source (`/private/tmp/intent-published-red.log`). The focused recovery/admission/deliver-first gate passed 257 cases across eight files; the final status/OpenAPI/WebMCP/function-calling checks passed 68 cases across five files. Typecheck and both dry-run Worker bundles passed. Fixtures use real disposable signatures and an intercepted processor on Base, Polygon, Arbitrum, World and Solana. No real payment or production deployment was made; GitHub owns the full suite.

Only the catalogue capture and protected-status substeps are checked off. BUY-017, BUY-034 and BUY-037 remain open. Next: durable reconciliation scheduling (including a crash before the legacy KV question is written), verified outcome propagation, and reconstruction/delivery from the original record. Commission/publication capture, acquiring a lost status handle after payment verification expires, legacy records, and other products' partial external operations remain unresolved. The current status intentionally does not claim delivery from a settlement record. A captured attempt with no outcome never times out into permission to charge again; automatic resolution is still to be built. This is a draft follow-up, not a claim that money safety is complete.
