...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  for (const id of planned) {
    ...
    const rawEvents = read('events.jsonl').trim().split('\n').filter(Boolean).map(JSON.parse);
    const events = normalizeClaudeEvents(rawEvents);
    let final, observed;
    try { final = parseClaudeFinal(read('final.txt')); } catch { /* Missing/malformed final fails. */ }
  ...
  const summary = { date: '2026-09-17', host: 'Claude Code', modelRequested: protocol.modelRequested,
    ...
    claudeGatePassed: results.length === planned.length && results.every((r) => r.passed), fullTwoHostGatePassed: false,
    codexGate: 'Revised Codex cohort retained at seven of eight strict passes; protocol-3 path fix is locally tested and observed in Claude, but not yet rechecked in Codex.',
    modelsReported: [...new Set(results.map(r => JSON.parse(fs.readFileSync(path.join(root,r.id,'run.json'))).modelReported))], results };
