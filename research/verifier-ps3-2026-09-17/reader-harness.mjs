...
const capture = new URL('./capture-verification.mjs', import.meta.url);
...
export function promptFor(cwd, scenario) {
...
export async function runAttempt({ id, scenario, attempt, frozen, output, model, timeoutMs = 180000 }) {
  ...
  fs.mkdirSync(record);
  const cwd = fs.mkdtempSync(`/private/tmp/scvd-reader-v2-${id}-`);
  ...
  const prompt = promptFor(cwd, scenario);
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const protocol = { version: 2, date: new Date().toISOString(), modelRequested: 'gpt-5.6-luna',
