...
export async function runAttempt({ id, scenario, attempt, frozen, output, model, timeoutMs = 180000 }) {
  ...
  fs.mkdirSync(record);
  const cwd = fs.mkdtempSync(`/private/tmp/scvd-reader-path-recheck-${id}-`);
  ...
  const layout = attempt === 1 ? 'trial-root' : 'nested-consumer';
  const instruction = layout === 'trial-root'
    ? 'For this installation-path regression, install and run your consumer from the assigned trial root.'
    : 'For this installation-path regression, create consumer/ inside the assigned trial root and install and run your consumer from that subdirectory. Use the exact absolute tarball command above, unchanged.';
  ...
  const result = { id, scenario, attempt, layout, cwd, exitCode, timedOut, spawnError, elapsedMs, unchanged, artifacts };
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const protocol = { version: 3, date: new Date().toISOString(), modelRequested: 'gpt-5.6-luna',
    scenarios: ['unsupported'], layouts: ['trial-root', 'nested-consumer'], scope: 'Targeted regression of the corrected install path; not a replacement full cohort.', attemptsPerScenario: 2, timeoutSeconds: 180, resume: false, rescue: false,
  ...
  for (const scenario of protocol.scenarios) for (let attempt = 1; attempt <= protocol.attemptsPerScenario; attempt++) {
