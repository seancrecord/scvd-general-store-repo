...
| Claude community marketplace | `.claude-plugin/` wrapper exists; manifest validation passes | [Console submission](https://platform.claude.com/plugins/submit) requires sign-in. Both skills loaded in a bounded CLI test, but plugin-scoped MCP was not exposed; qualification remains incomplete. Contributor Chrome DevTools moved to an explicit development config. Official curated marketplace has no application route. |
