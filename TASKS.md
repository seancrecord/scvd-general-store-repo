...
- [ ] "luckies" (renamed from LUCKY_CHARM, per keeper) — $5+ PWID custodial: keeper selects a small real object, names it, records provenance, assigns power in farmers-market register stated straight, GRADES vibe strength honestly (some are stronger; he says so), photographs, holds forever; patrons write in with results and luckies get promoted or BENCHED (couch treatment is real; the luck isn't always even). Register sample, canon-approved: "idk what it does but it's a fucking arrowhead." The store delights in its patrons' absurd wins and says so plainly. Copy states once: he knows they don't work; his OCD doesn't care; neither will yours. OPEN QUESTION for the keeper at build time: does luckies stand beside pet_rock or absorb it (a rock being one kind of lucky)? Machine's recommendation: absorb later, not now — the rock has certificates, featured-shelf billing, and cross-references (jar, drawer, hello note) that make it the store's most recognizable object; fold it in as "the classic lucky" once luckies has its own legs
