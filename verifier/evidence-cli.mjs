...
import { createEvidenceBundle, verifyEvidenceBundle, detachedTimestamp, EVIDENCE_BUNDLE_MAX_BYTES } from "./evidence-bundle.js";
...
const HELP = `scvd-evidence — free export and offline verification
  export <verify-response-url> --out <new-directory> [--evidence <local-file> ...]
...
async function localBytes(path) {
  ...
  try {
    ...
    if (!stat.isFile() || stat.size > EVIDENCE_BUNDLE_MAX_BYTES) throw new Error("file_not_regular_or_too_large");
    const buffer = Buffer.alloc(EVIDENCE_BUNDLE_MAX_BYTES + 1);
    let offset = 0;
    ...
    if (offset > EVIDENCE_BUNDLE_MAX_BYTES) throw new Error("file_too_large");
...
async function remoteJson(url) {
  ...
  try {
    for (;;) {
      ...
      if (done) break;
      length += value.length;
      if (length > EVIDENCE_BUNDLE_MAX_BYTES) throw new Error("source_too_large");
      chunks.push(value);
    }
...
async function main(args) {
  ...
  for (let i = 0; i < rest.length; i += 2) {
    ...
    if (!["--out", "--public-key", "--evidence"].includes(key) || !value || (key !== "--evidence" && flags[key])) throw new Error("invalid_arguments");
  ...
  if (command === "verify") {
    ...
    const result = await verifyEvidenceBundle(JSON.parse((await localBytes(input)).toString("utf8")), { publicKey: flags["--public-key"] });
  ...
  const response = await remoteJson(source);
  ...
  const attachments = [];
  for (const path of evidence) attachments.push({ name: basename(path), bytes: await localBytes(path) });
  const bundle = await createEvidenceBundle(response, { sourceUrl: source.href, capturedAt: new Date().toISOString(), issuerDocument, attachments });
