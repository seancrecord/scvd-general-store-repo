...
export function evidenceBytes(value) {
  if (typeof value !== "string" || value.length > EVIDENCE_BUNDLE_MAX_BYTES || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(value)) throw new Error("invalid_base64");
...
function bound(value) {
  ...
  if (!text || text.length > EVIDENCE_BUNDLE_MAX_BYTES || encoder.encode(text).length > EVIDENCE_BUNDLE_MAX_BYTES) throw new Error("bundle_too_large");
}
...
export async function createEvidenceBundle(response, options = {}) {
  ...
  for (const file of options.attachments ?? []) {
    if (!(file.bytes instanceof Uint8Array) || file.bytes.length > EVIDENCE_BUNDLE_MAX_BYTES) throw new Error("attachment_too_large");
  ...
  if (response.existence?.proof_base64) {
    evidenceBytes(response.existence.proof_base64);
  ...
  const bundle = {
    ...
    attachments,
    timestamp,
    context: { source_url: options.sourceUrl ?? null, captured_at: options.capturedAt ?? null, issuer_document: options.issuerDocument ?? null, response, authenticated: false },
  };
  bound(bundle);
  return bundle;
}
...
export async function verifyEvidenceBundle(bundle, options = {}) {
  ...
  try {
    ...
    for (const attachment of bundle.attachments) {
      ...
      const bytes = evidenceBytes(attachment.bytes_base64);
    ...
    if (bundle.timestamp !== null) {
      ...
      evidenceBytes(bundle.timestamp.proof_base64);
    }
