...
- **Receipt treaty — the first yes arrived (09-10).** StillOS
  Notary answered the drafted ask unprompted in issue #622,
  before you sent it: yes, their half published. The treaty
  relation is built on `/trust-list.json` (v2) and the roster
  is empty until you read their statement — the build's
  egress refused their host, so nobody here has. Three hands:
  1. TEST — open
     `https://stillosdigitalholdings.com/notary/trust?format=json`.
     Confirm it names our origin, our key URL
     (`/.well-known/scvd-signing-key`), and says what they
     honour our artifacts as. If it does, say "confirmed" and
     the entry lands dated that day (the shape is in
     `docs/RECEIPT_TREATY_ASK.md`, "The first yes"). If it
     does not, nothing lands and the reply says so.
  2. SEND — the reply drafted for your pen in the same
     section, on issue #622. It asks for their key URL and
     verify URL, which the issue did not give and our half
     needs.
  3. RULE — their offer of credit on a key after our house
  ...
  here — the PR text was outside this session's reach.
