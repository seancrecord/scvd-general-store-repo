...
export function dinoMark(
  ...
  return `<g transform="translate(${x} ${y}) scale(${scale.toFixed(5)})" fill="${fill}"${opacity === 1 ? "" : ` fill-opacity="${opacity}"`}><g transform="translate(0 100) scale(0.01118568 -0.01118568)"><path d="${DINO_PATH}"/></g></g>`;
}
...
export const FAVICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
  ...
  <g transform="${DINO_TRANSFORM}" fill="${FAVICON_MARK}">
    <path d="${DINO_PATH}"/>
  </g>
</svg>
`;
