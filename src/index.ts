...
/*
 * Everything a response passes through — the https redirect and HSTS,
 * the trailing dot and slash, the isolate's Server-Timing and its log
 * line, the 402's compression, Vary, CORS, conditional GETs, the house
 * headers, the porch visit — lives in src/lib/edge.ts now, in the order
 * it was always registered here, because the doors Worker
 * (src/doors.ts) must answer with the same edge. Registered from one
 * list so neither entry can drift from the other.
 */
...
app.route("/", desvelaRegistryRoutes);
app.route("/", shopWindowRoutes);
...
/*
 * The error handler lives in src/lib/edge.ts with the rest of the edge
 * (2026-09-05): a door check that throws in the doors Worker must fall
 * off the same shelf it falls off here.
 */
