...
- If that key ever changes: one key, NO successor key exists. A legitimate handover is announced at the key URL BEFORE the new key signs anything, and the announcement is itself signed by the OUTGOING key. A new key that has already issued artifacts, or a notice the old key did not sign, is not a handover \u2014 treat it as a compromise. Full form, and the failure case it does not cover, at ${base}/attestation
