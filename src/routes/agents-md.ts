...
${paymentConfig ? paymentNetworkGuide(paymentConfig) : paymentMethod()}
...
2. Request an item: GET ${base}/api/buy/{item_id} — the store answers HTTP 402 with the payment terms in the PAYMENT-REQUIRED header (base64 JSON), plus a plain-English note in the body. A bare request is answered too: required_params in the 402 body names any input the door needs. A request that supplies an invalid input receives a field refusal instead of terms. Prices and each input contract are also free at ${base}/menu/{item_id}?view=compact and ${base}/api/catalog/v1.
...
- The corpus — weekly signed observations of the x402 ecosystem, hash-chained and free to read, with timestamp status and proofs for independent Bitcoin verification: ${base}/corpus (data at ${base}/corpus.json, per-host at ${base}/corpus/host/{host}.json, and one host's changes as Atom at ${base}/feeds/host/{host}.xml).
