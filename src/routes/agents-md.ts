...
- The bounty board — the store PAYS you to shop: walk a listed x402 door with your own wallet, claim at POST ${base}/api/bounty-claim with the settlement transaction, and the door's price plus a finder's fee comes back as a signed EIP-3009 authorization you redeem yourself. The chain's part is verified against terms the store captured from that door's own 402 before a cent moves; your observations ride along as your own labeled claim. Board and rules: ${base}/api/bounties.
- Regulars' credit — 5% of every organic purchase banks to the wallet that paid, no account (the wallet is the card). Balance free at ${base}/api/credit/{wallet}; at $1 it cashes out in USDC to that same wallet. A closed-loop rebate: never transferable, not a token, idle balances expire, and the store's whole outstanding liability is published beside your balance.
