...
function corpusDatasetJsonLd(base: string): string {
  const dataset = {
    ...
    "@type": "Dataset",
    // The constants /corpus.json and the storefront already share; this
    // page carried a third, hand-typed copy until 2026-09-03.
    name: CORPUS_DATASET_NAME,
    description: CORPUS_DATASET_DESCRIPTION,
    url: `${base}/corpus`,
    identifier: corpusDatasetIdentityFields().identifier,
    sameAs: [`${base}/corpus.json`, ...corpusDatasetIdentityFields().sameAs],
...
function landingHtml(base: string, facts: WalletFacts | null): string {
  return `<section>
    ...
    <section>
      ...
      <p class="menu-desc">The week, READ rather than tabulated: <a href="/ledger">The Week&rsquo;s Ledger</a> &mdash; one page per signed week joining the counts, the movement against the week before, the defects by name, whether the feeds were even answering, and what this instrument could not see. Its findings are machine-derived from fields each one names, so every sentence can be recounted rather than taken. <code>/ledger/{week}.json</code> for the machine copy.</p>
      ...
      <p class="menu-desc"><strong>Subscribe.</strong> The latest signed snapshot at an address that never changes: <a href="/corpus/latest.json"><code>/corpus/latest.json</code></a>. Poll it with <code>If-None-Match</code> (the ETag is the sha256 of the bytes) or <code>If-Modified-Since</code> (Last-Modified is the snapshot's own date); a 304 costs nothing and a 200 is a new week. Then <code>/corpus/changes/{week}.json</code> says what moved — doors added and removed, recoveries and regressions, changed payment routes and prices, changed defect state, and <code>changed_pay_to</code>, where a door asks to be paid moving between two signed weeks (as digests, compared only where both weeks captured one) — as fields and as a plain changelog. A host the chain has never probed is not a dead end: the ask is queued by name and the next weekly sweep reads the host's own files for a door; the queue and where each host stands is <a href="/corpus/asked.json"><code>/corpus/asked.json</code></a>. The same weeks as Atom: <a href="/feeds/corpus.xml"><code>/feeds/corpus.xml</code></a>. A notebook that fetches the chain, recomputes every digest and counts with denominators, runnable top to bottom: <a href="${NOTEBOOK_URL}">examples/corpus-recompute.ipynb</a>.</p>
