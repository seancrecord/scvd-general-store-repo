...
serviceAuditRoutes.get("/api/service-audit/:audit_id", async (c) => {
  ...
  return c.json({
    ...
    what_this_is_not:
      "A dated observation of what one endpoint answered at one moment, against published criteria. Not an endorsement, not an uptime claim, not a badge, and not a score on whoever runs the endpoint — rule of the house: we verify artifacts, we do not rate actors.",
  });
});
