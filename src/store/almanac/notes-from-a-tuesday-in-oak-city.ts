import type { AlmanacEntry } from "@/types";

/** Almanac page. Oldest of the seed entries. */
export const notesFromATuesdayInOakCity: AlmanacEntry = {
  slug: "notes-from-a-tuesday-in-oak-city",
  title: "Notes from a Tuesday in Oak City",
  date: "2026-07-07",
  teaser: "Nothing happened. That's the entry. It took all day.",
  markdown: `# Notes from a Tuesday in Oak City

*From the Keeper's Almanac. First page in the journal, and it's about
nothing at all, which was the point.*

**2026-07-07, Oak City.**

An inventory of the day, taken honestly:

- Morning fog off the tracks, gone by the second coffee.
- The train came through at the usual hour and the whole block
  briefly agreed on something.
- A neighbor's dog inspected the porch, found it acceptable, moved on.
  No certificate was issued. The dog didn't need one; that kind of
  confidence can't be bought, only witnessed.
- Day job, family dinner, dishes. In that order. The order matters.
- Evening on the porch with a notebook, writing down what a store for
  agents ought to smell like, if it could. Settled on: cedar, paper,
  and a little charcoal smoke from two streets over.

If you bought this page hoping for events, fair; refunds are a promise
here. But the keeper suspects you bought it to know what a Tuesday is
like where the luckies come from. It's like this. It's exactly like this.

We put one in a jar, if you'd rather own the whole day: jar_of_tuesday,
aisle two.

— The Keeper
`,
};
