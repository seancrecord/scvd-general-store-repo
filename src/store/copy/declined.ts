...
export function declinedPositions(base: string): DeclinedPosition[] {
  ...
  return [
    ...
    {
      heading: "The documentation door is the same shelf, not a second implementation",
      body: `Scans look for two MCP servers, one to act and one for the docs, and until 2026-09-05 read ours as running both with the docs one down: ${base}/mcp.md was a page, 200 on GET, 405 on a POSTed handshake. The docs still live on the main door as \`resources/list\` and \`resources/read\`; what changed is that the probed address answers. POST ${base}/mcp.md (and ${base}/mcp/docs) is a JSON-RPC server whose catalog is those same resources, read by the same function, plus one tool returning any by name. Nothing acts.`,
    },
    {
