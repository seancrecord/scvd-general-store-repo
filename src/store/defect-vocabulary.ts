...
export const DEFECT_CLASSES: readonly DefectClass[] = [
  ...
  },
  {
    ...
    asserts:
      "A byte-identical, already-settled payment presented a second time is not served as a new sale: it is refused, or answered with the original purchase's own outcome naming the settlement that paid for it.",
    costs:
    ...
    detectable: "paid",
    our_signal: "launch_check stage: replay (outcome served_again)",
    falsified_by:
      "The endpoint answering the identical presented payment on the second attempt with a refusal, or with the original purchase naming its settlement transaction, at the stated moment.",
    also_known_as: [
      {
        instrument: "Cairn (cairnwake.com)",
    ...
    repair_hint:
      "Record each settled authorization nonce at settle time and, BEFORE fulfillment runs, either refuse a byte-identical presentation or answer it with the original purchase and its settlement reference, charged nothing — one keyed read at the till. Naming the original settlement either way keeps you clear of the nonce-unbound class below.",
    buyer_hint:
  ...
  {
    id: "re-challenges-spent-authorization",
    title: "Asks for new money when a settled payment is presented again",
    asserts:
      "A byte-identical, already-settled payment presented a second time is answered with the original outcome or a refusal, never with a fresh payment challenge.",
    costs:
      "A buyer who paid and lost the response re-presents the same authorization and is told to pay. Its nonce is spent, so the only way through the new challenge is a second signature — and the second signature is a second charge for goods the first one already bought. From the buyer's side it is one purchase attempted twice; from the seller's, two sales. This is the receiver-side defect the x402 specification thread measured in seven of ten money paths in September 2026.",
    detectable: "paid",
    our_signal: "launch_check stage: replay (outcome rechallenged)",
    falsified_by:
      "The endpoint answering the identical presented payment on the second attempt with anything other than a new payment challenge — a refusal, or the original purchase — at the stated moment.",
    sourced_by:
      "The x402 specification thread: aurumflux20's field read of ten money paths (x402-foundation/x402#3325, 2026-09-03) and the receiver obligation drafted from it (#3437). This store is the registrar, not the author.",
    registered: "2026-09-12",
    repair_hint:
      "When a presented authorization's nonce is already spent, look it up before you build a challenge: answer with the original purchase's outcome and its settlement reference (a paid retry, charged nothing), or refuse naming that settlement. Never let the spent-nonce path fall through to the code that issues a fresh 402.",
    buyer_hint:
      "If a door answers your re-presented payment with a new challenge, do not sign it: your first authorization may have settled, and a second one is a second charge. Keep the settlement reference from your first response, read the chain for it, and use the door's recovery path or ask for delivery against that settlement before paying again.",
  },
  {
    id: "advertised-version-unpayable",
    title: "Refuses payment in the protocol version it advertises",
    asserts:
      "A correctly signed payment, presented in the protocol version the door's own challenge advertises, reaches that door's verifier: it is accepted, or refused on something about the payment, never answered with the door's own terms re-served as though nothing had been presented.",
    costs:
