...
export const CORRECTIONS: readonly Correction[] = [
  {
    ...
    how_long:
      "From whenever the first non-EVM, non-Solana door entered the walk until 2026-09-04. The affected rounds are in the chain and are not being rewritten; the corrected walk publishes fresh verdicts and the old rows stay readable beside them, which is what the chain is for. How many earlier rounds carried the same error is recoverable from the corpus and has not yet been counted.",
    found_by:
      "The keeper asked how to double-check an operator who had replied \"I forsure know mine works\". Running our own preflight against his door returned 7 of 8 checks passing and one failure, and the failure was ours: his XRPL address read as a Solana one. Reading the round for the same signature found 63 hosts, three of them on our own outreach list — we had written to operators about doors our instrument had misjudged.",
    what_changed:
      "familyOf now names xrpl, stellar and algorand and returns \"unknown\" instead of defaulting to evm, and readPayTo gained a fourth verdict kind, unknown-network, whose payable is null: not judged, never failed. l3bChecks threads that null through, keeps unjudged entries out of the verdict, and prints them beside the pass so the reading names what it did not judge. amount-atomic no longer judges XRPL issued currencies or unknown chains, and transfer-method-signable no longer calls a non-EVM method unbuildable, since the methods it knows are EVM signature standards. test/value-checks.spec.ts holds the three real doors that were misjudged, by their actual addresses, and holds a genuinely broken EVM door still failing both checks.",
  },
  {
    date: "2026-09-03",
    what_was_wrong:
