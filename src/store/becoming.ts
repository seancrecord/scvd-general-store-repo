...
export const WATCHED: readonly Watched[] = [
  ...
  },
  {
    ...
    today:
      "The trigger fired before the ask was sent. 2026-09-10: StillOS Notary — an operator on the outreach register the keeper had not yet written to — read the drafted ask in the public repository and answered it from the other direction (issue #622): yes, with their half stated at a URL they control, shaped to mirror /trust-list.json and marked reciprocated: false, meaning their statement stands whether or not this store answers. What was built the same day: the treaty relation now exists on /trust-list.json (version 2) beside transacted and used, with the terms both sides commit to stated once on the list, counted apart, and a per-entry rule that a treaty points at the other operator's own statement URL and never paraphrases it. The first entry waited a day on purpose: their statement was reported in words the build could not check — the environment's egress refused their host — and an entry for a URL nobody here has opened would be exactly the wishful thinking the counterpart allowlist refuses. 2026-09-11: the keeper read it, it says what the issue said, and StillOS Notary is the first treaty entry the list has ever held — their statement URL, their keyring, their verify template, dated the day he read it. The reply went back on the issue the same day, on his word. Something bilateral exists. What this entry still watches: whether a second operator ever says yes, and whether either side's statement is still there when re-read.",
  },
  {
    /**
