...
export const WATCHED: readonly Watched[] = [
  ...
  },
  {
    ...
    today:
      "The trigger fired before the ask was sent. 2026-09-10: StillOS Notary — an operator on the outreach register the keeper had not yet written to — read the drafted ask in the public repository and answered it from the other direction (issue #622): yes, with their half stated at a URL they control, shaped to mirror /trust-list.json and marked reciprocated: false, meaning their statement stands whether or not this store answers. What was built the same day: the treaty relation now exists on /trust-list.json (version 2) beside transacted and used, with the terms both sides commit to stated once on the list, counted apart, and a per-entry rule that a treaty points at the other operator's own statement URL and never paraphrases it. What does not exist yet, and why: the first entry. Their statement was reported in words the build could not check — the environment's egress refused their host — and an entry for a URL nobody here has opened would be exactly the wishful thinking the counterpart allowlist refuses. It lands by the keeper's hand, dated the day he reads it; the reply to them is drafted for his pen in docs/RECEIPT_TREATY_ASK.md, and no outward send has happened.",
  },
  {
    /**
