 ...
 * twice.
 *
 * Two fields were their answer, not ours, and both fail CLOSED if
 * wrong: `timestamp_unit` (a millisecond clock read as seconds is
 * centuries in the future and refused on every call), and whether
 * the provider key is a separate secret from the signing secret.
 * Answered 2026-09-04: unix seconds, and two secrets, both generated
 * by Hal at listing creation. The row below matched before the
 * answer arrived; TRADE_COUNTER.md carries all ten.
 */
...
export interface TradePartner {
  ...
  /**
   * THE PARTNER'S OWN TERMS, in plain words, as they answered them
   * (pass eight). What their side does on a timeout, a refund, a
   * rotation, a price — printed on the account row so the second
   * reader of the contract (their engineer, our keeper, a customer
   * with a receipt) reads the same sentences. Never money math.
   */
  partner_terms?: readonly string[];
}
...
export const TRADE_PARTNERS: readonly TradePartner[] = [
  ...
  },
  {
    ...
    partner_share_bps: 500,
    settles_in:
      "sats over Lightning (OpenNode, mainnet), credited by Hal at 95% of each listing's fixed sats price; this statement is in USD",
    items: [
      "certificate_of_patronage",
      "context_anchor",
      "bitcoin_anchor",
    ...
    /** Hal's answers of 2026-09-04 to the ten questions, verbatim in substance. */
    partner_terms: [
      "Both secrets are generated by Hal when the seller creates a listing, one pair per listing: the provider key sent in X-Hal-Provider-Key and the HMAC signing secret. The seller saves each pair against its item on this side; nothing is issued from here.",
      "One listing per item, each with a concrete endpoint URL (the item's door below) and a fixed integer price in sats set at listing time. Hal does no USD-to-sats conversion per call.",
      "One POST per order with a 30-second timeout and no automatic retry; a fresh attempt carries a fresh timestamp, nonce and signature and is a new order on Hal's side. Hal does not add an order_ref to the body, so on this side too each attempt is a fresh sale.",
      "A failed call (error status, timeout, invalid JSON, oversized body) refunds Hal's buyer and credits nothing; a valid JSON delivery finalises the seller credit and Hal's fee. No clawback after delivery on either side.",
      "Rotation is a cutover: Hal replaces the secret and pauses the listing; the seller sets the new value here and then resumes. No two secrets are active at once on Hal's side.",
      "Hal publishes no fixed egress IPs and no bilateral per-order statement API; reconciliation is Hal's seller dashboard against this account's signed statement.",
...
export function tradeSecretNames(
  partner: Pick<TradePartner, "id">,
  itemId?: string,
): {
  signing: string;
  previous: string;
  ...
  /*
   * PER-LISTING SECRETS (pass ten, 2026-09-05). The first account
   * issues one provider key and one signing secret PER LISTING, not
   * per seller. So a secret may be keyed to an item as well as to
   * the account: TRADE_SECRET_<ID>__<ITEM>, read first; the account
   * level name, read second, still serves a partner that issues one
   * pair for everything. Two underscores, so an item id can never
   * collide with an account id's own suffixes.
   */
  const scope = itemId ? `${id}__${itemId.toUpperCase().replace(/[^A-Z0-9]/g, "_")}` : id;
  return {
    signing: `TRADE_SECRET_${scope}`,
    previous: `TRADE_SECRET_${scope}_PREVIOUS`,
    provider_key: `TRADE_PROVIDER_KEY_${scope}`,
  };
}
