...
export const CATALOG_ROW_SCHEMA = {
  ...
  properties: {
    ...
    buy_url: { type: "string", format: "uri", description: "The x402 door that sells it. A bare GET quotes; a door with required inputs refuses the PAID request without them — see buy_url_template." },
    buy_url_template: { type: "string", format: "uri", description: "Present only where the bare buy_url will not serve: the same door with its required inputs as <slot> placeholders. Replace each slot and buy that URL." },
