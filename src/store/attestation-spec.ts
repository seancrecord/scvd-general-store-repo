 ...
 */
import { EVIDENCE_TOOLS_DESCRIPTION } from "@/store/evidence-tools";
...
> = {
  ...
  third_party_observation: {
    ...
    means:
      "The store observes something it does not control and signs what it saw: public chain state on the networks each instrument declares, or whether somebody else's URL answered when we walked past it out of band. We are not a party to the thing being observed, which is the property that makes the observation worth anything.",
    weakness:
...
export const NOT_BUILT: readonly string[] = [
  ...
  "No post-quantum signatures in production. Production artifacts use Ed25519, which a cryptographically relevant quantum computer could forge. A real Ed25519 plus ML-DSA-65 checkpoint has been tested in an isolated experiment; it is not independently audited or enabled here. Production migration still needs multi-algorithm artifact and verifier support, key registration and an independently timestamped continuity record before changing the signer. A handover alone does not make old signatures quantum-resistant.",
