...
export function supportsObservationRecovery(item: MenuItem | undefined): boolean {
  return !!item && [
    ...
    "research_comparison", "spot_check", "provenance_check", "the_statement", "settlement_reconciliation",
...
export function supportsPersonalRecovery(item: MenuItem): boolean {
  return item.fulfillment === "instant" && ["the_confession", "coffees_for_closers", "graffiti_on_a_train", "luckies", "pack", "window_pick"].includes(item.id);
}
