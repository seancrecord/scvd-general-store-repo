...
export function readReason(raw: string): {
  ...
  if (reason.startsWith("local:input_missing:")) {
    ...
    return {
      fault: "buyer",
      reading: `A SIGNED request arrived without \`${param}\`, the input this door cannot work without, and was refused before the gate — no money moved. THE CHALLENGE NAMES IT NOW: since 2026-09-15 the resource description opens with "REQUIRES ?${param}=" ahead of the pitch, and the required-inputs extension beside it carries the parameter and a retry_url_template — both inside the PAYMENT-REQUIRED header every client decodes, verified live on all twelve input-taking doors on 2026-09-21. The 400 this buyer received carried required_params, the free input contract and a next_action too. So "they did not read the schema" is no longer the whole story, and neither is publishing it harder. One client doing this is theirs. The same thing from DIFFERENT clients is still ours, and what was ours was never the publishing: a stock x402 client probes the bare door, signs the challenge it got back and retries THE SAME URL, and nothing in the protocol tells it to retry another one — so the store was handing out a \`buy_url\` that refuses the purchase it advertises. Since 2026-09-21 every catalog row, the compact contract and this refusal itself carry \`buy_url_template\`: the same door with \`?${param}=<${param}>\` already in it, the identical string the 402's retry_url_template names. A row dated after that is a client that had a buyable URL in every document it read.`,
    };
  }
