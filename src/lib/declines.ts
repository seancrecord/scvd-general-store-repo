...
export function escalateSharedReasons(report: DeclineReport): SharedReason[] {
  ...
  for (const { reason, clients, escalated, machinery_only } of shared) {
    ...
    const note = escalated
      ? ` ESCALATED BY THE DESK: ${clients.length} DIFFERENT clients hit this same code in this window (${who}), which is the condition the sentence above names. Two implementations do not independently forget the same parameter. WHAT THE FIX IS NOW: the header has carried this requirement since 2026-09-15 and was checked carrying it on every input-taking door on 2026-09-21, so a fifth place to read it buys nothing. What is unfixed is the door's shape — it is payable only at a URL the buyer must build, and the stock loop retries the one it probed. Read the retry_url_template in the required-inputs extension beside these rows before reading any of them as a careless buyer.`
