...
export const SHELF_CLUSTERS: readonly ShelfCluster[] = [
  ...
  },
  {
    ...
    purpose:
      "Purpose: a signed settlement attestation for an x402 payment on Base, Polygon or Solana, a signed x402 conformance audit, x402 endpoint monitoring, a signed x402 payment client test, an x402 launch check, a Bitcoin timestamp, or a record of the authorization an agent claims to be acting under — the claimed instructions, a declared spending ceiling and a declared expiry, written down before it spends and held by neither party — have a disinterested third party go and look at something, or hold a dated record of what was claimed, then sign what it saw: whether a URL was still answering hours later, or what the chain actually says about a settlement. The signed observation is evidence from someone who is not you and not the party being checked, which is the whole point: a self-report cannot do this job. Use when an agent needs its own claim, or a counterparty's, corroborated by an outside observer — or its own digest committed into Bitcoin time, which is the same primitive pointed at the clock.",
    itemIds: [
      /**
