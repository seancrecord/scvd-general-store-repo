...
/* ---- the train, pulled up to the front (2026-08-29) ----
   The wall out back holds every approved tag, oldest first. This is
   the last few cars of it, and the day's biggest bid rides the head
   end. Graffiti has to READ: the paint is in the treatment — the
   steel, the rivets, the tilt, the spray glow — and the tag itself
   stays in a face a person can actually make out on a phone. Every
   car says what it is: bought. */
.train-strip {
  display: flex; gap: 0.7rem;
  overflow-x: auto; padding: 0.9rem 0.2rem 1rem;
  scrollbar-width: thin;
}
.train-car {
  flex: 0 0 auto; min-width: 12rem; max-width: 20rem;
  padding: 0.85rem 0.95rem 0.7rem;
  border: 1px solid #3a3050; border-radius: 3px;
  background:
    repeating-linear-gradient(180deg, transparent 0 9px, rgba(0,0,0,0.16) 9px 10px),
    linear-gradient(172deg, rgba(46,38,60,0.9), rgba(18,14,26,0.92));
  box-shadow: inset 0 0 26px rgba(0,0,0,0.6), 0 4px 14px rgba(0,0,0,0.35);
  transform: rotate(-0.3deg);
}
.train-car:nth-child(even) { transform: rotate(0.35deg); }
/* Rivets along the top edge of the boxcar. */
.train-car::before {
  content: ""; display: block; height: 4px; margin: -0.2rem 0 0.5rem;
  background-image: radial-gradient(circle, rgba(200,190,210,0.4) 0 1px, transparent 1px);
  background-size: 11px 4px;
}
.train-tag {
  font-family: 'Impact', 'Haettenschweiler', 'Arial Narrow Bold',
    'Franklin Gothic Bold', Georgia, sans-serif;
  font-size: 1.12rem; line-height: 1.25; letter-spacing: 0.01em;
  color: #f2e9dd;
  text-shadow: 0 0 10px rgba(93,230,200,0.28), 0 1px 0 rgba(0,0,0,0.7);
  overflow-wrap: anywhere;
}
.train-meta {
  margin-top: 0.5rem;
  font-family: ui-monospace, 'Courier New', monospace;
  font-size: 0.64rem; letter-spacing: 0.09em; color: var(--night-faded);
}
.train-meta a { text-decoration: none; border-bottom: 1px solid rgba(93,230,200,0.22); }
.train-meta a:hover { border-bottom-color: var(--neon-hot); }
/* The head end. Same car, more paint on it. */
.train-car-top {
  border-color: var(--neon); border-left: 2px solid var(--neon);
  box-shadow: inset 0 0 26px rgba(0,0,0,0.6), 0 0 18px rgba(255,180,94,0.16);
}
.train-car-top .train-tag { color: #fff4e6; text-shadow: 0 0 14px rgba(255,180,94,0.4), 0 1px 0 rgba(0,0,0,0.7); }
.train-top-label {
...
.train-paid {
  ...
  font-size: 0.7rem; letter-spacing: 0.05em; color: var(--night-faded);
}
.train .menu-meta { margin-top: 0.5rem; text-align: center; font-size: 0.78rem; color: var(--night-faded); }
.train { margin-top: 2.2rem; }
@media (prefers-reduced-motion: reduce) {
  .train-car, .train-car:nth-child(even) { transform: none; }
}
