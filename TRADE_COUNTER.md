...
2. **Two secrets, both generated by Hal** when the seller creates a
   listing: the provider key and the signing secret. NOTHING is
   minted on this side. The keeper creates a paused listing at
   `https://sell.halmarket.dev/services/new`, copies the two values
   Hal shows into `TRADE_SECRET_HAL` and `TRADE_PROVIDER_KEY_HAL`,
   and confirms to Hal only the listing id and `is_paused: true`.
   Open: whether Hal issues one pair per seller account or one per
   listing. One per listing means secrets keyed per item on this
   side, a small change; asked.
